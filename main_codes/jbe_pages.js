...
function func_promo(promo,stockno){
  //alert(stockno);    
  axios.post('../../main_codes/api/z_stock.php', { clientno:CURR_CLIENT, request: 30,
    promo: promo,
    stockno: stockno
  },JBE_HEADER)
...
function post_comm(v,stockno){
  ...
  //alert(stockno);
  axios.post('../../main_codes/api/z_comment.php', { clientno:CURR_CLIENT, request: 2,
    comment: v,
    usercode: CURR_USER,
    username: CURR_NAME,
    stockno: stockno
...
function del_comm(id,stockno){
  MSG_SHOW(vbYesNo,"CONFIRM: ","Delete this comment?",function(){
    axios.post('../../main_codes/api/z_comment.php', { clientno:CURR_CLIENT, request: 4,
      id: id
    })
    .then(function (response) {     
      console.log(response.data); 
