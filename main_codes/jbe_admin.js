...
function chk_lognow(){
  ...
  showProgress(true);
  axios.post('../../main_codes/api/z_user.php',{ clientno:CURR_CLIENT, request: 101,     
    userid: u, 
    pword: p 
  },JBE_HEADER)     
...
function fm_admin(){
  ...
  var menuEditProfile=
    '<div onclick="fm_profile(2)" style="width:100%;height:40px;margin-top:20px;padding:5px;cursor:pointer;background:none;">'+
      '<img src="../../main_../../main_gfx/avatar.png" style="float:left;height:100%;"/>'+
...
function chk_fld(u,p,vmode){
  ...
  axios.post('../../main_codes/api/z_user.php',{ clientno:CURR_CLIENT, request: 101, userid: u, pword:p },JBE_HEADER)     
  .then(function (response) {     
    console.log(response.data);  
    if(response.data.length > 0){
...
function save_profile(){
  ...
  showProgress(true);
  axios.post('../../main_codes/api/z_user.php', { clientno:CURR_CLIENT, request: req, 
      usercode:usercode,
      userid:u,
      pword:p,
      username:n, 
    ...
    dtl+=
      '<div style="width:100%;height:40px;margin-top:10px;padding:0px;background:none;">'+
        '<div style="float:left;height:100%;width:45px;background:none;">'+          
          '<img src="../../app/'+CURR_SITE+'/upload/users/'+aryDB[i]['usercode']+'.jpg?'+n+'" style="float:left;height:100%;width:40px;border-radius:50%;border:2px solid black;background:none;"/>'+
        '</div>'+
...
function chgLevel(usercode,usertype){  
  showProgress(true);      
  axios.post('../../main_codes/api/z_user.php', { clientno:CURR_CLIENT, request: 301, 
    usercode: usercode,
    usertype: usertype
  },JBE_HEADER)
  .then(function (response) {           
...
function del_staff(usercode){
  ...
  MSG_SHOW(vbYesNo,"CONFIRM: ","Are you sure to Delete user: "+username+"?",function(){
    showProgress(true);      
    axios.post('../../main_codes/api/z_user.php', { clientno:CURR_CLIENT, request: 4, 
      usercode: usercode,
      photo: photo
    },JBE_HEADER)
    .then(function (response) {
