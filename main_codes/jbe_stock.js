...
function saveStock(){
  ...
  showProgress(true);
  axios.post('../../main_codes/api/z_stock.php', { clientno:CURR_CLIENT, request:req,    
    stockname: stockname,
    descrp: descrp,
    price: jnumber(price),
  ...
  .then(function (response) {     
    ...
    if(thisFile){
      ...
      uploadNOW(thisFile,newName,'upload/',ob);  
      showItems();
    }
...
function delStock(){  
  ...
  MSG_SHOW(vbYesNo,"CONFIRM: ","Are you sure to Delete this Stock?",function(){
    showProgress(true);  
    axios.post('../../main_codes/api/z_stock.php', { clientno:CURR_CLIENT, request: 4,    
      photo:photo,    
      stockno: stockno,
      dir:dir
    },JBE_HEADER)
