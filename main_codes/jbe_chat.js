...
function markAsRead(){
  ...
  //showProgress(true);        
  axios.post('../../main_codes/api/z_chat.php', { clientno:CURR_CLIENT, request: 5, usercode: usercode, sender:sender },JBE_HEADER)
...
function sendMsg(){
  ...
  if(msg != '' || newName != ''){    
    showProgress(true);
    axios.post('../../main_codes/api/z_chat.php', { clientno:CURR_CLIENT, request: 2,
      usercode: usercode,
      admin: v_admin,
      trano: trano,
      trantype: 0,
...
function getChats(){  
  ...
  DB_CHAT=[];  
  showProgress(true);    
  axios.post('../../main_codes/api/z_chat.php', { clientno:CURR_CLIENT, request: 1, usercode: usercode }) 
  .then(function (response) { 
    showProgress(false);
    console.log(response.data);
...
function delChat(v_trano){  
  ...
  MSG_SHOW(vbYesNo,"CONFIRM:","Are you sure to Delete this Item?",
    function(){   
      showProgress(true);    
      axios.post('../../main_codes/api/z_chat.php', { clientno:CURR_CLIENT, request: 4,
        trano: v_trano,
        usercode: usercode,
        f_owner:f_owner        
      },JBE_HEADER)
...
function disp_chat_owner(){  
  mnu_chat_owner();
  showProgress(true);
  axios.post('../../main_codes/api/z_chat.php', { clientno:CURR_CLIENT, request: 0 }) 
  .then(function (response) { 
    showProgress(false);
    console.log(response.data);
    DB_CHAT = response.data;    
